...
object MyNil extends ImmLinkedList[Nothing] {
  ...
  override def head = throw ArrayIndexOutOfBounds("MyNil has no head.")
  override def tail = throw ArrayIndexOutOfBounds("MyNil has no tail.")

  def length = 0
  def apply(index: Int) = throw ArrayIndexOutOfBounds("Index argument must be between 0 and size.") 
}